...
MODEL_PATH = os.path.join(RESULTS_DIR, 'best_densenet_model.h5')
INDICES_PATH = os.path.join(RESULTS_DIR, 'class_indices.json')

def predict_image(image_path):
    """
    Predicts the class of an input image using the trained DenseNet121 model.
    """
    ...
    # IMPORTANT: Use DenseNet121 preprocess_input!
    ...
    print("-" * 30)
    print(f"Prediction Result (DenseNet121):")
    ...
    plt.title(f"DenseNet: {predicted_class} ({confidence:.2%})")
    plt.axis('off')
    plt.show()
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        # Default path (same as before, or you can change it)
